...
// Sample highlights for preview (matches schema's jsonb highlights field)
...
// Mock post matching the database schema (SelectPost type)
const mockPost: SelectPost = {
  id: "preview-mock-1",
  title: "Electric Vehicles Dominate Singapore October 2025",
  slug: "ev-october-2025-analysis",
...
|-----------|---------------|------------|
| Electric | 2,081 | 52.60% |
| Petrol-Electric | 1,351 | 34.15% |
| Petrol | 432 | 10.92% |
| Others | 92 | 2.33% |
...
The trend towards electrification shows no signs of slowing down.`,
  excerpt:
    "Singapore's car market saw a remarkable shift towards electrification, with Electric Vehicles (EVs) securing over half of all new registrations.",
  heroImage:
  ...
  modifiedAt: new Date("2025-11-12T00:00:00Z"),
  publishedAt: new Date("2025-11-12T00:00:00Z"),
};
...
// Preview-specific display data (computed values, not part of schema)
const previewDisplayData = {
  readingTime: "5 min read",
  views: "313 views",
};

// Preview-specific table data (for demonstrating table styling)
const fuelTableColumns = [
...
// Helper to format date from schema's timestamp field
  ...
  <div className="min-h-screen">
    ...
    <div className="relative mb-12 aspect-[4/3] w-full overflow-hidden md:aspect-[21/9]">
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-12">
        ...
        <div className="flex flex-wrap items-center gap-2 font-medium text-white/70 text-xs uppercase tracking-wider drop-shadow-md sm:gap-4 sm:text-sm">
          ...
          <span>{previewDisplayData.readingTime}</span>
          <span className="h-1 w-1 rounded-full bg-white/50" />
          <span>{previewDisplayData.views}</span>
        </div>
      </div>
    </div>
    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-1 gap-0 border-black border-t-2 md:grid-cols-2 lg:grid-cols-3">
        {(mockPost.highlights as Highlight[])?.map((item) => (
          <div
    ...
    <section className="mb-12">
      ...
      <div className="border-primary border-l-4">
        ...
        >
          <TableHeader columns={fuelTableColumns}>
            {(column) => (
              <TableColumn
                key={column.key}
